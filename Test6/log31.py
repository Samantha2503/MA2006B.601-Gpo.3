...
if st.session_state.on_landing_page:
    ...
    st.markdown('<h2 class="landing-subtitle">Casa Monarca</h2>', unsafe_allow_html=True)
    ...
    if not st.session_state.logged_in:
        ...
        with tab1:
            ...
            if valid_private_keys and valid_public_keys:
                ...
                area_seleccionada = st.selectbox(
                    "Área:", 
                    ["(Ninguna)", "Finanzas", "TI", "RRHH", "Legal", "Marketing"], 
                    key="area_global"
                )
                st.markdown("----")
                ...
                plantilla = st.selectbox("📄 Selecciona una plantilla:", list(TEMPLATES.keys()))

                if plantilla != "Selecciona una plantilla" and datos_destinatarios:
                    try:
                        primer_nombre = datos_destinatarios[0]["nombre"]
                        asunto = TEMPLATES[plantilla]["asunto"].format(nombre=primer_nombre)
                        cuerpo = TEMPLATES[plantilla]["cuerpo"].format(
                    ...
                    st.info(f"🧠 Se cargó la plantilla: *{plantilla}*. Puedes modificar el contenido antes de enviarlo.")
                else:
                    asunto, cuerpo = "", ""
                ...
                for i, etapa in enumerate(st.session_state.flujo_etapas):
                    ...
                    with col2:
                        area = st.selectbox(
                            ...
                            ["(Ninguna)", "Finanzas", "TI", "RRHH", "Legal", "Marketing"],
                            index=["(Ninguna)", "Finanzas", "TI", "RRHH", "Legal", "Marketing"].index(etapa["area"]) if etapa["area"] else 0,
                            key=f"area_{i}"
                        )
                        if i > 0:
                ...
                if st.button("Encrypt, Sign and Send"):
                    ...
                    if not datos_destinatarios:
                        ...
                        if not encrypted_signed_body.ok:
                            ...
                            for d in datos_destinatarios:
                                if plantilla != "Selecciona una plantilla":
                                    asunto_final = TEMPLATES[plantilla]["asunto"].format(nombre=d["nombre"])
                                    cuerpo_final = TEMPLATES[plantilla]["cuerpo"].format(
                                        nombre=d["nombre"],
                                        fecha=datetime.today().strftime("%d/%m/%Y"),
                                        hora=datetime.now().strftime("%H:%M")
                                    )
                                else:
                                    asunto_final = asunto  # texto manual escrito en input
                                    cuerpo_final = cuerpo
        ...
        with tab_admin:
            ...
            if st.button("Registrar usuario"):
                ...
                cursor.execute(
                    "INSERT OR IGNORE INTO usuarios (nombre, email, rol, area, contraseña) VALUES (?, ?, ?, ?, '1234')",
            ...
            st.subheader("🔁 Rutas automáticas de reenvío")
            with sqlite3.connect("firmas.db") as conn:
                cursor = conn.cursor()

                st.markdown("### ➕ Agregar nueva ruta")
                intermediario = st.text_input("Email del intermediario (quien autoriza)", key="ruta_intermediario")
                destinatario = st.text_input("Email del destinatario final (a quien se reenviará)", key="ruta_destinatario")   
