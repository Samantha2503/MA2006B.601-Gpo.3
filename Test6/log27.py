    ...
    with tab1:
        ...
        if valid_private_keys and valid_public_keys:
            ...
            if st.button("Encrypt, Sign and Send"):
                if not all([subject, body, recipient_email, area_destino]):
                    ...
                    if not encrypted_signed_body.ok:
                        ...
                        if archivo:
                            ...
                            try:
                                ...
                                if tmp_original_file_path and os.path.exists(tmp_original_file_path):
                                    os.remove(tmp_original_file_path)
                        ...
                        success_email, msg_email = send_email_with_attachment(
                            subject=subject,
                            body_text=final_body_for_email, # Use modified body for email
                        ...
                        if success_email:
                            st.success("✅ Correo cifrado enviado.")
                            ...
                            guardar_interaccion(
                                ...
                                link_archivo=link_archivo, # Link to Drive for logging
                                area_destino=area_destino
                            )
                            if st.session_state.drive:
                                log_text = (
                                    f"{datetime.utcnow().isoformat()} - Enviado a {recipient_email} - Asunto: {subject}\n"
                                    f"Mensaje:\n{body}\n\nPGP:\n{str(encrypted_signed_body)}\n"
                                    f"Link Archivo Firmado en Drive: {link_archivo if link_archivo else 'N/A'}"
                                )
                                save_log_to_drive(
                                    st.session_state.drive,
                                    f"log_envio_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.txt",
                                    log_text,
                                    area_destino=area_destino,
                                    aceptado=True
                                )
                        ...
                        if archivo_to_attach_path and os.path.exists(archivo_to_attach_path):
                            os.remove(archivo_to_attach_path)
